...
export default function DynamicDashboard({ charts, onClose, activeDataset, toggleTheme, isDark }) {
 ...
 const [activePreset, setActivePreset] = useState(localStorage.getItem('lumina_layout') || '3col');
  ...
  const [layoutKey,        setLayoutKey]        = useState(0); // ✅ layout reset key
  ...
  // ✅ Reset layout + key on filter/preset change to prevent overlap
  useEffect(() => {
  ...
  const applyPreset = p => {
    setActivePreset(p);
    localStorage.setItem('lumina_layout', p); // <-- ADD THIS LINE
  ...
  const captureGrid = async () => {
    ...
    setExportWidth(firstPanel ? firstPanel.offsetWidth - 24 : 380); // padding consideration
    ...
    const canvas = await html2canvas(gridEl, {
      ...
      backgroundColor: isDark ? '#0e0e0d' : '#f9f9f8',
...
const handleExportPDF = async (name) => {
    ...
    try {
        ...
        // --- NEW: Grab the pinned charts from local storage ---
        ...
        const response = await axios.post(`${API_URL}/exports/generate-pdf`, 
            { 
                ...
                layout: activePreset,
                // Send the storage data to the backend!
                userData: userData,
                pinnedCharts: pinnedChartsData,
                pinKey: pinKey
            },
            { 
                headers: { Authorization: `Bearer ${token}` },
  ...
  const handleExportPPT = async name => {
    ...
    try {
      ...
      cover.background = { color: isDark ? '0e0e0d' : 'f9f9f8' };
      cover.addText('Lumina Intelligence', { x: 0, y: '40%', w: '100%', align: 'center', fontSize: 40, color: isDark ? 'eeeeec' : '111110', bold: true });
      cover.addText(name || activeDataset?.name || 'Analytics Report', { x: 0, y: '55%', w: '100%', align: 'center', fontSize: 18, color: '6f6e69' });
      
      const slide = pres.addSlide();
      slide.background = { color: isDark ? '0e0e0d' : 'f9f9f8' };
      slide.addText(name || 'Dashboard Overview', { x: 0.5, y: 0.3, w: '90%', fontSize: 20, color: isDark ? 'eeeeec' : '111110', bold: true });
  ...
  const chartTypeLabels = { bar: <BarChart3 size={13} />, line: <LineChart size={13} />, pie: <PieChart size={13} />, table: <TableIcon size={13} /> };
    ...
    <div className={`dd-root ${isDark ? 'dark' : ''}`}>
      ...
      <header className="dd-header">
        <div className="flex items-center gap-3">
          <button onClick={onClose} className="dd-back-btn">
            <ArrowLeft size={14} />
            <span>Back</span>
          </button>
          <div style={{ width: 1, height: 20, background: 'var(--dd-border)' }} />
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--dd-text-1)' }}>{activeDataset?.name || 'Analytics'}</div>
            <div style={{ fontSize: 11, color: 'var(--dd-text-3)' }}>{visibleCharts.length} panels</div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="dd-filter-bar">
              {[
                ...
                <button key={t.k} onClick={() => setChartFilter(t.k)} className={`dd-tool-btn ${chartFilter === t.k ? 'active' : ''}`} title={t.k}>{t.i}</button>
              ))}
            </div>

            {/* Layout picker */}
            <div className="relative" ref={dropdownRef}>
              <button onClick={() => setIsLayoutOpen(!isLayoutOpen)} className="dd-btn-outline" style={{ gap: 6 }}>
                <LayoutDashboard size={13} />
                <span style={{ fontSize: 11 }}>{activePreset === 'smart' ? 'Smart' : activePreset === '1col' ? '1 Col' : `${activePreset.charAt(0)} Col`}</span>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setIsPresentMode(!isPresentMode)} className="dd-btn-outline" style={{ padding: '0 10px' }} title="Present">
            <Eye size={13} />
          </button>
          <button onClick={toggleTheme} className="dd-btn-outline" style={{ padding: '0 10px' }}>
            {isDark ? <Sun size={13} /> : <Moon size={13} />}
          </button>
          <button onClick={handleShare} className="dd-btn-outline">
            {copiedShare ? <CheckCircle2 size={13} /> : <Share2 size={13} />}
            <span>Share</span>
          </button>
          <button onClick={() => { setExportType('pdf'); setIsExportModalOpen(true); }} disabled={isExportingPDF} className="dd-btn-outline">
            {isExportingPDF ? <Loader2 size={13} className="animate-spin" /> : <Download size={13} />}
            <span>PDF</span>
          </button>
          <button onClick={() => { setExportType('ppt'); setIsExportModalOpen(true); }} className="dd-btn-primary" style={{ background: '#ea580c' }}>
            <MonitorPlay size={13} />
            <span>PPT</span>
          </button>
          <button onClick={() => setIsConsultOpen(!isConsultOpen)} className="dd-btn-primary">
            <Sparkles size={13} />
            <span>AI</span>
          </button>
        </div>
              ...
              <div key={String(c.id)} id={`panel-${c.id}`} className="dd-panel group">
                <div className="dd-panel-header drag-handle">
                  ...
                  <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => setPinnedInsights(p => p.includes(String(c.id)) ? p.filter(x => x !== String(c.id)) : [...p, String(c.id)])}
                    ...
                    <button onClick={() => handleExportImage(String(c.id))} className="dd-panel-action" title="Export PNG"><ImageIcon size={12} /></button>
                    <button onClick={() => setRemovedPanels(p => [...p, String(c.id)])} className="dd-panel-action" style={{ '--hover-color': '#ef4444' }} title="Remove"><Trash2 size={12} /></button>
                  </div>
                </div>
                ...
                {c.explanation && (
                  <div className={`dd-insight-overlay ${pinnedInsights.includes(String(c.id)) ? 'dd-pinned-insight' : ''}`}>
                    <div className="flex items-start gap-2">
                      ...
                      <p style={{ margin: 0, fontSize: 12, lineHeight: 1.5, color: 'var(--dd-text-2)' }}>{c.explanation}</p>
                    </div>
                  </div>
                )}
              </div>
        ...
        {isConsultOpen && (
          <aside className="dd-sidebar animate-in">
            <div className="dd-sidebar-header">
              <div className="flex items-center gap-2">
                <div style={{ width: 28, height: 28, borderRadius: 6, background: 'var(--dd-bg)', border: '1px solid var(--dd-border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <button onClick={() => setIsConsultOpen(false)} className="dd-panel-action"><X size={14} /></button>
            </div>
            ...
            <div className="dd-sidebar-footer">
              <div className="dd-input-group">
                ...
                />
                <button onClick={handleSend} className="dd-send-btn"><Send size={13} /></button>
              </div>
            </div>
          </aside>
        )}
      ...
      {/* ── Present mode exit ── */}
      {isPresentMode && (
        <button onClick={() => setIsPresentMode(false)} className="dd-exit-present-btn" style={{ position: 'fixed', bottom: 32, left: '50%', transform: 'translateX(-50%)', zIndex: 2000 }}>
      ...
      {/* ── Export Modal ── */}
      {isExportModalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', padding: 16 }}>
          <div style={{ background: 'var(--dd-surface)', border: '1px solid var(--dd-border)', borderRadius: 12, padding: 32, maxWidth: 400, width: '100%', boxShadow: '0 24px 48px rgba(0,0,0,0.12)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              {exportType === 'pdf' ? <FileText size={18} style={{ color: 'var(--dd-text-2)' }} /> : <MonitorPlay size={18} style={{ color: 'var(--dd-text-2)' }} />}
              <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--dd-text-1)' }}>Export {exportType.toUpperCase()}</span>
            </div>
            <p style={{ fontSize: 13, color: 'var(--dd-text-3)', margin: '0 0 24px' }}>
              {visibleCharts.length} panels will be exported. Give this report a name.
            </p>

            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 500, color: 'var(--dd-text-3)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Report Title</label>
              <input
                type="text"
                value={reportName}
                ...
                autoFocus
                style={{ width: '100%', padding: '10px 14px', background: 'var(--dd-bg)', border: '1px solid var(--dd-border)', borderRadius: 8, fontSize: 13, color: 'var(--dd-text-1)', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1px solid var(--dd-border)', background: 'none', color: 'var(--dd-text-2)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                Cancel
              </button>
              <button
                ...
                style={{ flex: 2, padding: '10px', borderRadius: 8, border: 'none', background: 'var(--dd-accent)', color: 'var(--dd-bg)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: (isExportingPDF || !reportName.trim()) ? 0.4 : 1 }}
              >
                {isExportingPDF ? <Loader2 size={14} className="animate-spin" /> : 'Export'}
              </button>
            </div>
          </div>
        </div>
