...
import {
  ...
  Sun, Moon, Monitor, LogOut, Zap, Save,
...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sw-btn.link {
    ...
    color:var(--accent-txt); padding:0; font-size:12px; font-weight:500;
  ...
  .sw-nav-btn {
    ...
    font-family:'DM Sans',sans-serif; cursor:pointer;
    transition:all .12s; width:100%; text-align:left; margin-bottom:4px;
  }
  ...
  .sw-mobile-nav-item.active {
    ...
    box-shadow:0 2px 8px rgba(59,63,206,.30);
  }
  .sw-mobile-nav-item.dnav { color:var(--danger); border-color:var(--danger-bdr); background:var(--danger-dim); }
  ...
  .sw-drop-list   { max-height:260px; overflow-y:auto; padding:6px; }
  .sw-drop-item {
  ...
  @media (max-width: 640px) {
    ...
    .sw-theme-btn { padding:10px 6px; font-size:11px; }

    .sw-key-box { padding:12px 14px; }

    .sw-mobile-nav-fade-right {
      pointer-events:none;
      position:sticky;
      right:0;
      width:40px;
...
───────────────────────────────────────────── */
const PROVIDERS = [
  { id:'gemini',     name:'Google Gemini',  model:'Gemini 2.5 Flash',      tag:'Free tier',            bg:'#e8f0fe',fg:'#1a73e8',initials:'GG', isDefault:true,  keyUrl:'https://aistudio.google.com/app/apikey',         keyDomain:'aistudio.google.com' },
  { id:'openai',     name:'OpenAI',         model:'GPT-4o / o1',           tag:'High reasoning',       bg:'#d1fae5',fg:'#065f46',initials:'OA',                  keyUrl:'https://platform.openai.com/api-keys',           keyDomain:'platform.openai.com' },
  { id:'claude',     name:'Anthropic',      model:'Claude 3.7 Sonnet',     tag:'Coding & analysis',    bg:'#fde8e0',fg:'#7c2d12',initials:'AN',                  keyUrl:'https://console.anthropic.com/settings/keys',    keyDomain:'console.anthropic.com' },
  { id:'grok',       name:'xAI',            model:'Grok 3',                tag:'Real-time knowledge',  bg:'#e5e7eb',fg:'#111827',initials:'XA',                  keyUrl:'https://console.x.ai/',                          keyDomain:'console.x.ai' },
  { id:'deepseek',   name:'DeepSeek',       model:'DeepSeek R2',           tag:'Math reasoning',       bg:'#e0e7ff',fg:'#1e1b4b',initials:'DS',                  keyUrl:'https://platform.deepseek.com/api_keys',         keyDomain:'platform.deepseek.com' },
  { id:'mistral',    name:'Mistral AI',     model:'Mistral Large',         tag:'Open weights',         bg:'#fef3c7',fg:'#78350f',initials:'MI',                  keyUrl:'https://console.mistral.ai/api-keys/',           keyDomain:'console.mistral.ai' },
  { id:'perplexity', name:'Perplexity',     model:'Sonar Pro',             tag:'Online search AI',     bg:'#d1fae5',fg:'#064e3b',initials:'PP',                  keyUrl:'https://www.perplexity.ai/settings/api',         keyDomain:'perplexity.ai' },
  { id:'openrouter', name:'OpenRouter',     model:'GPT-4o via OpenRouter', tag:'Access 200+ models',   bg:'#f3e8ff',fg:'#6b21a8',initials:'OR',                  keyUrl:'https://openrouter.ai/keys',                     keyDomain:'openrouter.ai' },
  { id:'groq',       name:'Groq Cloud',     model:'Llama 3.3 70B',         tag:'Ultra-fast inference', bg:'#fef3c7',fg:'#92400e',initials:'GQ',                  keyUrl:'https://console.groq.com/keys',                  keyDomain:'console.groq.com' },
];
...
const ProfileSection = ({ user, loading, onSave }) => {
  ...
  const save = async () => {
    ...
    } catch(e) { show('danger',e.response?.data?.error||'Failed to save profile'); }
  ...
  return (
    <>
      ...
      <div className="sw-card" style={{display:'flex',alignItems:'center',gap:18,padding:'18px'}}>
        ...
        <span className="sw-badge accent" style={{padding:'4px 11px',fontSize:'11.5px', marginBottom:'25px'}}>Free plan</span>
      </div>
...
const SecuritySection = () => {
  ...
  const[vis,setVis]=useState(false);const[saving,setSaving]=useState(false);const[twoFA,setTwoFA]=useState(false);
  const {toast,show}=useToast();
  ...
  const changePw=async()=>{
    ...
    try{
      ...
      show('success','Password updated successfully');setCur('');setNw('');setCnf('');
  ...
  return (
    <>
      ...
      <div className="sw-card">
        ...
        {twoFA&&<div className="sw-alert info" style={{marginTop:12,marginBottom:0}}><Info size={13} style={{flexShrink:0,marginTop:1}}/>QR code setup will be available in the next release.</div>}
      </div>
    </>
  );
};
...
const NotificationsSection = ({ user }) => {
  ...
  const[p,setP]=useState(defaultPrefs);const[saving,setSaving]=useState(false);
  const s=k=>v=>setP(x=>({...x,[k]:v}));const{toast,show}=useToast();
  useEffect(()=>{if(user?.notification_prefs&&Object.keys(user.notification_prefs).length>0){setP({...defaultPrefs,...user.notification_prefs});}},[user]);
  const save=async()=>{
    setSaving(true);
    try{const token=localStorage.getItem('token');await axios.patch(`${API_URL}/auth/notifications`,p,{headers:{Authorization:`Bearer ${token}`}});show('success','Preferences saved');}
    catch{show('danger','Failed to save preferences');}finally{setSaving(false);}
  };
  return (
    <>
      <Toast toast={toast}/>
...
const AiSection = ({ user, onSave }) => {
  const[keys,setKeys]=useState({});const[provider,setProvider]=useState('gemini');
  const[search,setSearch]=useState('');const[open,setOpen]=useState(false);
  const[showKey,setShowKey]=useState(false);const[saving,setSaving]=useState(false);const[saved,setSaved]=useState(false);
  const dropRef=useRef(null);const{toast,show}=useToast();

  useEffect(()=>{if(user){if(user.ai_keys)setKeys(user.ai_keys);if(user.preferred_provider)setProvider(user.preferred_provider);}},[user]);
  ...
  useEffect(()=>{
    const fn=e=>{if(open&&dropRef.current&&!dropRef.current.contains(e.target)){setOpen(false);setSearch('');}};
  ...
  const handleSave=async()=>{
    ...
    try{
      const token=localStorage.getItem('token');
      const res=await axios.patch(`${API_URL}/auth/keys`,{ai_keys:keys,preferred_provider:provider},{headers:{Authorization:`Bearer ${token}`}});
      onSave?.(res.data.user);setSaved(true);show('success','AI engine configuration saved');setTimeout(()=>setSaved(false),3000);
    }catch(e){show('danger',e.response?.data?.error||'Failed to save configuration');}
    finally{setSaving(false);}
  };
    ...
    <>
      ...
      <div className="sw-card" style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div style={{display:'flex',alignItems:'center',gap:13}}>
          <div style={{width:42,height:42,borderRadius:11,background:'var(--accent-dim)',border:'1px solid var(--accent-bdr)',display:'flex',alignItems:'center',justifyContent:'center',color:'var(--accent-txt)'}}>
            <Sparkles size={18}/>
          </div>
          <div>
            <p style={{fontSize:14,fontWeight:600,color:'var(--text-1)',marginBottom:2}}>AI Engine Registry</p>
            <p style={{fontSize:12,color:'var(--text-3)'}}>Choose which AI model powers your workspace.</p>
          </div>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:7,padding:'5px 13px',borderRadius:999,background:'var(--success-dim)',border:'1px solid var(--success-bdr)'}}>
      ...
      <div className="sw-card">
        ...
        <div ref={dropRef} style={{position:'relative',marginBottom:14}}>
          <button type="button" onClick={()=>{setOpen(o=>!o);setSearch('');}} style={{width:'100%',display:'flex',alignItems:'center',gap:12,padding:'10px 12px',borderRadius:'var(--r-md)',background:'var(--field)',border:`1px solid ${open?'var(--accent)':'var(--field-bdr)'}`,cursor:'pointer',transition:'border-color .15s, box-shadow .15s',boxShadow:open?'0 0 0 3px var(--accent-dim)':'none'}}>
          ...
          {open&&(
            <div className="sw-dropdown sw-fadein">
              ...
              <div className="sw-drop-list">
                {filtered.length===0&&<p style={{padding:'10px 12px',fontSize:13,color:'var(--text-3)',textAlign:'center'}}>No providers found</p>}
                {filtered.map(p=>(
                  <div key={p.id} className={`sw-drop-item${p.id===provider?' sel':''}`} onClick={()=>pick(p.id)}>
                    ...
                    {p.id===provider?<CheckCircle2 size={14} color="var(--accent-txt)"/>:p.isDefault?<span style={{fontSize:10,fontWeight:600,color:'var(--success)',background:'var(--success-dim)',padding:'1px 7px',borderRadius:999,border:'1px solid var(--success-bdr)'}}>Free</span>:null}
                  </div>
                ))}
              </div>
            </div>
        ...
        {active.isDefault?(
          ...
          <div className="sw-key-box">
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10,flexWrap:'wrap',gap:8}}>
              ...
              <a href={active.keyUrl} target="_blank" rel="noopener noreferrer" className="sw-btn link">Get key at {active.keyDomain}<ArrowUpRight size={11}/></a>
            </div>
            <div className="sw-fwrap">
              <input className="sw-field sw-mono" type={showKey?'text':'password'} value={keys[provider]||''} onChange={e=>setKeys(k=>({...k,[provider]:e.target.value}))} placeholder={`Paste your ${active.name} secret key…`} style={{paddingRight:38,fontSize:13}}/>
              <button type="button" className="sw-ficon" onClick={()=>setShowKey(p=>!p)}>{showKey?<EyeOff size={14}/>:<Eye size={14}/>}</button>
            </div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
          <p style={{fontSize:12,color:'var(--text-3)'}}>Changes apply to all future AI queries.</p>
          <button className="sw-btn primary" onClick={handleSave} disabled={saving}>
            {saving?<><span className="sw-spin"/> Saving…</>:saved?<><CheckCircle2 size={13}/> Saved</>:<><Save size={13}/> Save configuration</>}
          </button>
        </div>
      </div>
      ...
      {configured.length>0&&(
        <div className="sw-card">
          ...
          {configured.map(([id,val])=>{
            const p=PROVIDERS.find(x=>x.id===id);if(!p)return null;
            return(
              <div key={id} className="sw-row">
                <div style={{display:'flex',alignItems:'center',gap:11}}>
                  ...
                  <div>
                    ...
                    <code className="sw-mono" style={{fontSize:11,color:'var(--text-3)'}}>{val.slice(0,8)}{'•'.repeat(10)}</code>
                  </div>
                </div>
                <button className="sw-btn danger" style={{padding:'5px 10px',fontSize:11.5}} onClick={()=>setKeys(k=>{const n={...k};delete n[id];return n;})}>
...
const BillingSection = ({ user }) => {
  const[stats,setStats]=useState({datasets:0,storage:0,queries:0});const[loading,setLoading]=useState(true);
  const{toast,show}=useToast();
  useEffect(()=>{
    const fetchStats=async()=>{
      try{const token=localStorage.getItem('token');const res=await axios.get(`${API_URL}/auth/billing-stats`,{headers:{Authorization:`Bearer ${token}`}});setStats(res.data);}
      catch(err){console.error(err);show('danger','Failed to load usage statistics');}finally{setLoading(false);}
    };fetchStats();
  },[]);
  ...
  return (
    <>
      <Toast toast={toast}/>
      <div className="sw-card sw-billing-plan-row" style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div>
          ...
          <p style={{fontSize:13,color:'var(--text-2)'}}>{isPro?'$19 / month · renews April 18, 2025':'Limited to 20 datasets · 500 MB storage'}</p>
        </div>
        {isPro
          ?<div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className="sw-btn">Manage billing</button><button className="sw-btn danger">Cancel plan</button></div>
      ...
      <div className="sw-card">
        ...
        {loading
          ? <><Skel h={40} mb={15}/><Skel h={40} mb={15}/><Skel h={40}/></>
          : usage.map(item=>{const pct=Math.min(100,(item.used/item.limit)*100);return(
              <div key={item.label} style={{marginBottom:15}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
                  <span style={{fontSize:12.5,color:'var(--text-2)'}}>{item.label}</span>
                  <span style={{fontSize:12.5,fontWeight:600,color:'var(--text-1)'}}>{item.used}{item.unit}<span style={{color:'var(--text-3)',fontWeight:400}}> / {item.limit}{item.unit}</span></span>
                </div>
...
───────────────────────────────────────────── */
const DangerSection = () => {
  const[password,setPassword]=useState('');const[deleting,setDeleting]=useState(false);const[showPassword,setShowPassword]=useState(false);
  const{toast,show}=useToast();const navigate=useNavigate();const{logout,token}=useStore();

  const handleDeleteAccount=async()=>{
    if(!password||password.trim()===''){show('danger','Please enter your password to confirm deletion.');return;}
    setDeleting(true);
    try{
      await axios.delete(`${API_URL}/auth/delete-account`,{headers:{Authorization:`Bearer ${token}`},data:{password}});
      show('success','Account deleted. Redirecting…');setTimeout(()=>{logout();navigate('/login');},1500);
    }catch(err){console.error(err);show('danger',err.response?.data?.error||'Account deletion failed.');setDeleting(false);}
  };

  const handleSignOutEverywhere=()=>{logout();navigate('/login');};
      ...
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        <div className="sw-card">
          ...
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,flexWrap:'wrap'}}>
            <div style={{flex:1,minWidth:0}}>
              <p style={{fontSize:13.5,fontWeight:500,color:'var(--text-1)',marginBottom:3}}>Sign out everywhere</p>
              <p style={{fontSize:12.5,color:'var(--text-3)',lineHeight:1.6}}>Immediately invalidates all active sessions across every device.</p>
            </div>
            <button className="sw-btn" onClick={handleSignOutEverywhere} style={{flexShrink:0,marginTop:2}}>
              <LogOut size={13}/> Sign out everywhere
            </button>
          </div>
        </div>
        <div className="sw-card" style={{borderColor:'var(--danger-bdr)'}}>
          <p className="sw-sec-title" style={{color:'var(--danger)',opacity:.75}}>Delete Account</p>
          <div style={{display:'flex',alignItems:'flex-start',gap:12,padding:'13px 15px',borderRadius:'var(--r-md)',background:'var(--danger-dim)',border:'1px solid var(--danger-bdr)',marginBottom:18}}>
            <AlertCircle size={15} color="var(--danger)" style={{flexShrink:0,marginTop:1}}/>
            <div>
              <p style={{fontSize:13.5,fontWeight:600,color:'var(--danger)',marginBottom:3}}>Permanently delete this account</p>
              <p style={{fontSize:12.5,color:'var(--danger)',opacity:.75,lineHeight:1.55}}>All SQL tables, saved queries, charts, and account history will be permanently erased. This cannot be undone.</p>
            </div>
          </div>
          <div style={{marginBottom:14}}>
            <label className="sw-lbl">Enter your password to confirm</label>
            <div className="sw-fwrap">
              <input className="sw-field" type={showPassword?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter your password" style={{paddingRight:38,borderColor:password?'var(--danger-bdr)':undefined,transition:'border-color .15s'}} disabled={deleting}/>
              <button type="button" className="sw-ficon" onClick={()=>setShowPassword(p=>!p)}>{showPassword?<EyeOff size={14}/>:<Eye size={14}/>}</button>
            </div>
            <p style={{marginTop:5,fontSize:11.5,color:'var(--text-3)',lineHeight:1.5}}>We need to verify your identity before deleting your account.</p>
          </div>
          <button onClick={handleDeleteAccount} disabled={!password||deleting} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'9px 16px',borderRadius:'var(--r-md)',border:`1px solid ${password&&!deleting?'var(--danger)':'var(--danger-bdr)'}`,background:password&&!deleting?'var(--danger)':'var(--danger-dim)',color:password&&!deleting?'#fff':'var(--danger)',fontSize:13,fontWeight:600,fontFamily:"'DM Sans',sans-serif",cursor:password&&!deleting?'pointer':'not-allowed',opacity:!password||deleting?0.55:1,transition:'all .15s'}}>
            {deleting?<><span style={{width:13,height:13,borderRadius:'50%',display:'inline-block',flexShrink:0,border:'2px solid rgba(255,255,255,.3)',borderTopColor:'#fff',animation:'sw-spin .6s linear infinite'}}/>Deleting account…</>:<><Trash2 size={13}/>Permanently delete my account</>}
          </button>
          {password&&!deleting&&<p style={{marginTop:10,fontSize:11.5,color:'var(--danger)',opacity:.6,textAlign:'center',lineHeight:1.5}}>Clicking above will immediately begin deletion. There is no confirmation step.</p>}
        </div>
      </div>
    </>
  );
...
───────────────────────────────────────────── */
const MobileNav = ({ active, setActive, onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (!nav) return;
    const activeEl = nav.querySelector('.sw-mobile-nav-item.active');
    if (activeEl) {
...
const Settings = () => {
  ...
  return (
    <div className="sw">
      ...
      <div style={{ padding: '20px 20px 160px', width: '100%', minHeight: '100vh' }}>
        <div style={{ maxWidth: 880, margin: '0 auto' }}>

          {/* Header */}
          <div className="sw-header-row" style={{ marginBottom: 12, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
          ...
          {/* Mobile scrollable nav — shown only on small screens via CSS */}
          <div style={{ margin: '0 -20px 16px' }}>
            <MobileNav active={active} setActive={setActive} onLogout={onLogout} />
          </div>
